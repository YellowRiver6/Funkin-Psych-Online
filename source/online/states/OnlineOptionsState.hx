...
class OnlineOptionsState extends MusicBeatState {
    ...
    override function create() {
		...
		#if DISCORD_ALLOWED
		DiscordClient.changePresence("菜单中", "在线设置");
		#end
		...
		items.add(nicknameOption = new InputOption("昵称", "在此设置你的游戏昵称！", ["男友"], (text, _) -> {
		...
		if (Auth.authID == null && Auth.authToken == null) {
			...
			items.add(registerOption = new InputOption("注册在线账号",
					"加入在线网络，将你的歌曲回放提交到排行榜！", ["用户名", "邮箱"], (text, input) -> {
					try {
					if (input == 0) {
			...
			items.add(loginOption = new InputOption("登录在线账号",
				"输入你的邮箱，等待一次性登录验证码！", ["me@example.org"], (mail, _) -> {
				try {
			...
			lastOption = recentOption;
			var recentOption:InputOption;
			items.add(recentOption = new InputOption("静音房间邀请",
			...
			items.add(sidebarOption = new InputOption("打开侧边栏", "打开在线功能侧边栏" + ((!controls.mobileControls) ? "，如果你无法打开的话。\n(按 " + InputFormatter.getKeyName(cast(ClientPrefs.keyBinds.get('sidebar')[0], FlxKey)) + " 键可随时打开！)" : ""), null, () -> {
			...
			items.add(deleteOption = new InputOption("删除在线账号", "再见！", null, () -> {
			...
			items.add(logoutOption = new InputOption("退出在线账号", "退出当前登录的在线账号", null, () -> {
