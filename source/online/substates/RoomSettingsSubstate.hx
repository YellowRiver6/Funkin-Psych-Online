...
class RoomSettingsSubstate extends MusicBeatSubstate {
	...
	override function create() {
		...
		items.add(publicRoom = new Option("公开房间", "启用后，房间会在搜索列表中公开显示。", () -> {
		...
		items.add(networkOnlyRoom = new Option("仅登录玩家加入", "启用后，只有注册用户可以加入此房间。", () -> {
		...
		items.add(anarchyMode = new Option("自由选择模式", "启用后，所有玩家都拥有房主权限。", () -> {
		...
		items.add(allPlayersChoose = new Option("让玩家选择", "启用后，所有玩家都可以选择歌曲与场景。", () -> {
		...
		items.add(swapSides = new Option("BF阵营", "切换到BF（玩家）一侧进行游戏。", () -> {
		...
		items.add(teamMode = new Option("团队模式", "以团队形式竞技，分数会与队友平均计算。", () -> {
		...
		items.add(royalMode = new Option("单谱面模式", "所有玩家在同一个谱面上游戏！", () -> {
		...
		items.add(royalModeDadSide = new Option("单谱面-对手阵营", "启用后，所有玩家在对手侧进行游戏。", () -> {
		...
		items.add(hideGF = new Option("隐藏GF", "在场景中隐藏GF角色。", () -> {
		...
		items.add(disableSkins = new Option("禁用皮肤", "禁止玩家使用自定义皮肤。", () -> {
		...
		items.add(winCondition = new Option("胜利条件", "...", () -> {
			...
			if (GameClient.room.state.winCondition != prevCond) {
				switch (GameClient.room.state.winCondition) {
					case 0:
						winCondition.descText.text = '准度最高的一方获胜！';
					case 1:
						winCondition.descText.text = '分数最高的一方获胜！';
					case 2:
						winCondition.descText.text = '失误最少的一方获胜！';
					case 3:
						winCondition.descText.text = 'FP最高的一方获胜！';
					case 4:
						winCondition.descText.text = '连击数最高的一方获胜！';
				}
		...
		items.add(modifers = new Option("游戏修改器", "在这里设置游戏玩法修改器！", () -> {
			close();
			FlxG.state.openSubState(new GameplayChangersSubstate());
		}, null, 0, 0, false, true));

		items.add(stageSelect = new Option("选择场景", "当前选择： " + (GameClient.room.state.stageName == "" ? '(默认)' : GameClient.room.state.stageName), () -> {
		...
		items.add(skinSelect = new Option("选择皮肤", "在这里选择你的自定义皮肤！", () -> {
		...
		items.add(gameOptions = new Option("游戏设置", "打开引擎主设置界面！", () -> {
