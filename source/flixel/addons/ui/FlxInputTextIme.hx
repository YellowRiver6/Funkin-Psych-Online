...
class FlxInputTextIme extends FlxText
{
	                    ...
	                    BackgroundColor:Int = FlxColor.WHITE, EmbeddedFont:Bool = true)
	{
		...
		Lib.current.stage.window.onTextInput.add(onTextInputIME);
		Lib.current.stage.window.onTextEdit.add(onTextEditIME);
	...
	override public function destroy():Void
	{
		...
		Lib.current.stage.window.onTextInput.remove(onTextInputIME);
		Lib.current.stage.window.onTextEdit.remove(onTextEditIME);
	...
	override public function update(elapsed:Float):Void
	{
		...
		// Poll IME state for candidate display
		#if (windows && cpp)
		if (hasFocus) {
			if (TSFBridge.hasComposition()) {
				var candCount:Int = TSFBridge.getCandidateCount();
				if (candCount > 0) {
					var pageStart:Int = TSFBridge.getPageStart();
					var pageSize:Int = TSFBridge.getPageSize();
					var sel:Int = TSFBridge.getSelectedIndex();
					var ds:String = pageStart > 0 ? "< " : "";
					for (i in 0...candCount) {
						if (i == sel && TSFBridge.hasComposition()) ds += "[";
						ds += (pageStart + i + 1) + "." + TSFBridge.getCandidate(i);
						if (i == sel && TSFBridge.hasComposition()) ds += "]";
						ds += " ";
					}
					...
					var candH:Float = _imeCandText.height > 0 ? _imeCandText.height : 20;
					if (y + height + candH + 4 > FlxG.height)
						_imeCandText.y = y - candH - 4;
					else
						_imeCandText.y = y + height + 4;
					// Clamp X to keep within screen
	...
	private function onTextInputIME(text:String):Void
	{
		if (!hasFocus) return;
		// Remove any active composition text before committing
		if (_imeCompStart >= 0 && _imeCompLen > 0) {
			this.text = this.text.substring(0, _imeCompStart) + this.text.substring(_imeCompStart + _imeCompLen);
			caretIndex = _imeCompStart;
			_imeCompStart = -1;
			_imeCompLen = 0;
			_imeOrig = null;
		}
		var t:String = filter(text);
		if (t.length > 0 && (maxLength == 0 || (this.text.length + t.length) <= maxLength))
		{
			this.text = insertSubstring(this.text, t, caretIndex);
			caretIndex += t.length;
			onChange(INPUT_ACTION);
		}
	}

	private var _imeCompStart:Int = -1;
	private var _imeCompLen:Int = 0;
	private var _imeOrig:String = null;

	private function onTextEditIME(text:String, start:Int, length:Int):Void
	{
		if (!hasFocus) return;
		// Remove previous composition text if any
		if (_imeCompStart >= 0 && _imeCompLen > 0) {
			this.text = this.text.substring(0, _imeCompStart) + this.text.substring(_imeCompStart + _imeCompLen);
			caretIndex = _imeCompStart;
			_imeCompStart = -1;
		...
		// Composition ended — commit
		if (text.length == 0) { _imeOrig = null; updateTextInputRect(); return; }
		// Save original text before first composition insert
		if (_imeOrig == null) _imeOrig = this.text;
		// Insert composition text at caret
		_imeCompStart = caretIndex;
		_imeCompLen = text.length;
		this.text = insertSubstring(this.text, text, _imeCompStart);
		caretIndex = _imeCompStart + _imeCompLen;
		updateTextInputRect();
	}

	private function updateTextInputRect():Void
	{
		var window = Lib.current.stage.window;
		if (window == null) return;

		var cx:Float, cy:Float, ch:Float = size + 2;

		if (text.length > 0 && caretIndex > 0)
		{
			var b = getCharBoundaries(caretIndex - 1);
			if (b != null) { cx = x + b.right; cy = y + b.top; ch = b.height; }
			else { cx = x + 2; cy = y + 2; }
		}
		else if (text.length > 0 && caretIndex < text.length)
		{
			var b = getCharBoundaries(caretIndex);
			if (b != null) { cx = x + b.left; cy = y + b.top; ch = b.height; }
			else { cx = x + 2; cy = y + 2; }
		}
		else { cx = x + 2; cy = y + 2; }

		#if !js
		cx -= textField.scrollH;
		#end

		window.setTextInputRect(new LimeRectangle(cx, cy, 1, ch));

		#if (windows && cpp)
		var sx:Float = window.width / FlxG.width;
		var sy:Float = window.height / FlxG.height;
		TSFBridge.updatePosition(Std.int(cx * sx), Std.int(cy * sy), Std.int(1 * sx), Std.int(ch * sy));
		#end
	}
	...
	private function set_hasFocus(newFocus:Bool):Bool
	{
		...
		else
		{
			...
			if (hasFocus) {
				...
				_imeCompLen = 0;
				_imeOrig = null;
				if (_imeCandText != null) _imeCandText.visible = false;
			}
		}
