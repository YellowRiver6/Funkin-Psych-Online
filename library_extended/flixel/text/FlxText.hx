...
class FlxText extends FlxSprite
{
	...
	//same as the setFormat()
	public function setFormatAsString(?Font:String, Size:Int = 8, Color:FlxColor = FlxColor.WHITE, ?AlignmentString:String, ?BorderStyle:FlxTextBorderStyle,
			BorderColor:FlxColor = FlxColor.TRANSPARENT, EmbeddedFont:Bool = true):FlxText
	{
		var Alignment:FlxTextAlign = null;
		if (AlignmentString == 'LEFT') Alignment = LEFT;
		if (AlignmentString == 'RIGHT') Alignment = RIGHT;
		if (AlignmentString == 'CENTER') Alignment = CENTER;
		if (AlignmentString == 'JUSTIFY') Alignment = JUSTIFY;
		BorderStyle = (BorderStyle == null) ? NONE : BorderStyle;
